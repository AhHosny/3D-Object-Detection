...
while not src_dir.endswith("scripts"):
...
class Demo_KittiDataset(Dataset):
    def __init__(self, configs):
        ...
        #self.sample_id_list = [float(os.path.basename(fn)[:-4]) for fn in self.sample_id_list] #kitti
        self.sample_id_list = np.arange(start=1, stop=250, step=1) #Ahosny
    ...
    def get_image(self, idx):
        ...
        img_path = os.path.join(self.image_dir, '{}.jpg'.format(idx)) #Ahosny
        img = cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2RGB)

        return img_path, img

    def get_lidar(self, idx):
        #lidar_file = os.path.join(self.lidar_dir, '{:010d}.bin'.format(idx)) #kitti
        lidar_file = os.path.join(self.lidar_dir, '{}.bin'.format(idx)) #Ahosny
