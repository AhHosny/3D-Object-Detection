"""
# Description: Demon script for both front view and back view
"""
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for sample_idx in range(len(demo_dataset)):
            ...
            write_credit(out_img, (50, 410), text_author='Cre: Ahmed Hosny', org_fps=(900, 410), fps=fps)

            if out_cap is None:
                out_cap_h, out_cap_w = out_img.shape[:2]
                fourcc = cv2.VideoWriter_fourcc(*'MJPG')
                out_path = os.path.join(configs.results_dir, '{}_2_sides.avi'.format(configs.foldername))
